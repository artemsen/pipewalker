...
:	_btn_ok(render::btn_ok, render::btn_ok_s, PW_BUTTON_X1, PW_BUTTON_Y),
	...
	_wrap_mode (true, -0.6f, -0.5f, 0.7f),
	_sound_mode(true, -0.6f, -1.7f, 0.7f),
	_prev_theme(render::btn_prev, render::btn_prev_s, -2.3f, -3.0f, 0.7f),
	_next_theme(render::btn_next, render::btn_next_s,  2.3f, -3.0f, 0.7f),
	_sett_size(level::sz_normal),
	_sett_wrap(true),
	_sett_sound(true)
{
	_map_size[0].setup(false, -0.6f, 2.7f, 0.7f, level::sz_small);
	_map_size[1].setup(false, -0.6f, 2.0f, 0.7f, level::sz_normal);
	_map_size[2].setup(false, -0.6f, 1.3f, 0.7f, level::sz_big);
	_map_size[3].setup(false, -0.6f, 0.6f, 0.7f, level::sz_extra);
}
...
bool mode_settings::draw(const float alpha)
{
	...
	renderer.draw_text(-4.6f,  4.3f, 0.9f, alpha, "Game settings");

	renderer.draw_text(-3.6f,  3.0f, 0.7f, alpha, "Map:");
	renderer.draw_text( 0.0f,  3.0f, 0.7f, alpha, "Small");
	renderer.draw_text( 0.0f,  2.3f, 0.7f, alpha, "Normal");
	renderer.draw_text( 0.0f,  1.6f, 0.7f, alpha, "Big");
	renderer.draw_text( 0.0f,  0.9f, 0.7f, alpha, "Extra");

	renderer.draw_text( 0.0f, -0.2f, 0.7f, alpha, "Wrap mode");

	renderer.draw_text(-4.8f, -1.4f, 0.7f, alpha, "Sound:");

	renderer.draw_text(-1.4f, -2.7f, 0.7f, alpha, "Theme");
	renderer.draw_text(0.0f - ((static_cast<float>(strlen(theme_name)) / 2.0f) * 0.55f), -3.55f, 0.7f, alpha, theme_name);
...
void mode_settings::initialize(const level::size sz, const bool wrap, const bool sound)
{
...
{
	initialize(_sett_size, _sett_wrap, _sett_sound);
}
